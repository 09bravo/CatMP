...
impl<T: Pod + neoffmpeg_rs::frame::audio::Sample, R: RbRef> FFMpegToCPalSampleForwarder
    for ringbuf::Producer<T, R>
where
    <R as RbRef>::Rb: RbWrite<T>,
{
    fn forward(
        ...
        Box::pin(async move {
            let cpal_sample_data: &[T] = audio_frame.plane(0);
...
impl FFmpegToCPalForwarder {
    fn new<T: Send + Pod + SizedSample + 'static + neoffmpeg_rs::frame::audio::Sample>(
